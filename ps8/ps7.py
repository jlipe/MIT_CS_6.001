...
class SimpleVirus(object):
    ...
    def reproduce(self, popDensity):
        ...
        randN = random.random()
        if randN < reproduce_probability:
...
class SimplePatient(object):
    ...
    def update(self):
        ...
        for virus in viruses_after_doesClear:
            ...
            try:
                new_virus = virus.reproduce(pop_density)
...
def simulationWithoutDrug():
    ...
    maxPop = 1000
    P1 = SimplePatient(viruses, maxPop)
    xaxis = range(0, 301)
    yaxis = [len(viruses)]
    for t in xaxis:
        if t == 0:
            ...
            viruses_in_patient = P1.update()
            yaxis.append(viruses_in_patient)

    pylab.plot(xaxis, yaxis)
    pylab.xlabel("Time Step")
    pylab.ylabel("Number of Viruses in Patient")
    pylab.title("Virus in Patient vs Time")
    pylab.show()
